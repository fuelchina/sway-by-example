<script lang="ts">
  ...
  import hexContent from './out/release/vector.hex?raw';
  import abiContent from './out/release/vector-abi.json?raw';
  import storageSlotsContent from './out/release/vector-storage_slots.json?raw';
  ...
  $store = {
    ...
    messages: {},
    getters: {
      storage_vec_examples: async () => {
        const { value } =  await contract.functions.storage_vec_examples().get()
        return value;
      },
      heap_vec_examples: async () => {
        const { value } =  await contract.functions.heap_vec_examples().get()
        return value;
      },
    },
