<script lang="ts">
  ...
  import hexContent from './out/release/compound-types.hex?raw';
  import abiContent from './out/release/compound-types-abi.json?raw';
  import storageSlotsContent from './out/release/compound-types-storage_slots.json?raw';
  ...
  $store = {
    ...
    getters: {
      test_func: async () => {
        ...
        return {x: value.x.toNumber(), y: value.y.toNumber()};
      },
    },
