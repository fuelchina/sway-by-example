<script lang="ts">
  ...
  import hexContent from './out/release/storage-map.hex?raw';
  import abiContent from './out/release/storage-map-abi.json?raw';
  import storageSlotsContent from './out/release/storage-map-storage_slots.json?raw';
  ...
  $store = {
    ...
    messages: {},
    getters: {
      basic_examples: async () => {
        const { value } =  await contract.functions.basic_examples().get()
        return value;
      },
      nested_examples: async () => {
        const { value } =  await contract.functions.nested_examples().get()
        return value;
      },
    },
