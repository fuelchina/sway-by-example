<script lang="ts">
  ...
  import hexContent from './out/release/control-flow-if.hex?raw';
  import abiContent from './out/release/control-flow-if-abi.json?raw';
  import storageSlotsContent from './out/release/control-flow-if-storage_slots.json?raw';
  ...
  $store = {
    ...
    getters: {
      "test_func(5)": async () => {
        const { value } =  await contract.functions.test_func(5).get()
        return value.toNumber();
      },
      "test_func(10)": async () => {
        const { value } =  await contract.functions.test_func(10).get()
        return value.toNumber();
      },
    },
